...
export default {
  ...
  methods: {
    ...
    handleSubmit (sid, interfaceName) {
      this.$spin()
      if (this.selectedProtocol === 'dhcp') {
        const tempSid = this.$uci.add('network_interfaces', 'interface')
        this.$uci.set('network_interfaces', tempSid, 'name', interfaceName)
        this.$uci.set('network_interfaces', tempSid, 'protocol', this.selectedProtocol)
        this.$uci.del('network_interfaces', sid)
        this.$uci.save().then(() => {
          this.handleTableRefresh()
        })
      } else this.handleTableRefresh()
